...
import { AppRoute, MAX_NEARBY_OFFERS_COUNT, RequestStatus } from '../../const';
...
import { selectNearbyOffers, selectNearbyOffersFetchingStatus } from '../../store/nearby-offers-data/selectors';
...
function OfferPage() {
  ...
  const offerInfoLoading = useAppSelector(selectOfferFetchingStatus);
  const nearbyOffers = useAppSelector(selectNearbyOffers);
  const nearbyDataLoading = useAppSelector(selectNearbyOffersFetchingStatus);
  ...
  if (offerInfoLoading === RequestStatus.Loading || nearbyDataLoading === RequestStatus.Loading) {
    return (<LoadingPage />);
  }

  if (offerInfoLoading === RequestStatus.Error) {
    return <Navigate to={AppRoute.NotFound} />;
  }

  if(offerInfoLoading !== RequestStatus.Success || !offerInfo) {
    return null;
  }
